...
function OtpSec(props) {
 ...
 //  console.log(locationData.state.phoneNumber);
  ...
  const verifyOtp = () => {
    alert(confirmOtp);
    alert(phoneNumber);
    axios.post('http://112.196.98.174:3000/api/v1/login', {
      phone: phoneNumber,
      otp: confirmOtp
    })
        ...
        <form>
          <div className="mb-3 mt-3">
            ...
            <input
              ...
              id="otp"
              name="otp" value={confirmOtp} onChange={inputHandler}
            />
            </div>
            
          </div>
          <button type="button" onClick={verifyOtp} className="btn btn-primary w-100">
          Verify
          </button>
        </form>
      </div>
